...
:MAKE_BUILD_EXE
  ...
  mv BUILD_WIN32\Xbmc\XBMC.exe BUILD_WIN32\Xbmc\XBMCMC.exe 
  ...
  ECHO ------------------------------------------------------------
  rem call buildpvraddons.bat %NET%
...
:NSIS_EXE
  ...
  SET XBMCMC_SETUPFILE=XBMCMCSetup-%GIT_REV%-%target%.exe
  ECHO Creating installer %XBMCMC_SETUPFILE%...
  IF EXIST %XBMCMC_SETUPFILE% del %XBMCMC_SETUPFILE% > NUL
  ...
  IF NOT EXIST "%NSISExePath%" (
    rem set to default path
    SET NSISExePath=E:\Projects\C\XBMC\DakaiTV\DakaiMC\project\Win32BuildSetup\tools\NSIS\Unicode\NSIS.exe
  )
  ...
  IF NOT EXIST "%XBMCMC_SETUPFILE%" (
	  set DIETEXT=Failed to create %XBMCMC_SETUPFILE%. NSIS installed?
	  goto DIE
  )
  ECHO ------------------------------------------------------------
  ECHO Done!
  ECHO Setup is located at %CD%\%XBMCMC_SETUPFILE%
  ECHO ------------------------------------------------------------
  GOTO VIEWLOG_EXE
